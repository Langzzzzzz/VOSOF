...
from my_functions import warp_image, generate_warped_image
...
def Run_video(dataset,video, num_frames, num_objects,model,Mem_every=None, Mem_number=None):
    ...
    for t in range(1,num_frames):
        ...
        # step 4 combine the optical flow result with the last frame segementation result
        estimate_curr_E = normalized_photometric_loss_map.unsqueeze(0).unsqueeze(0).unsqueeze(0).to(device) * E_last.to(device)
        estimate_curr_E = estimate_curr_E.squeeze(2) # torch.Size([1, 11, 480, 854])
...
if __name__ == "__main__":
    ...
    def get_arguments():
        ...
        parser.add_argument("-D", type=str, help="path to data",default='/root/autodl-tmp/data/youtube-vos')
    ...
    Trainset = Youtube_MO_Train('{}train/'.format(DATA_ROOT))
    Testloader = data.DataLoader(Trainset, batch_size=1, num_workers=1,shuffle = True, pin_memory=True)
