...
def Run_video(dataset,video, num_frames, num_objects,model,Mem_every=None, Mem_number=None):
    ...
    for t in range(1,num_frames):
        ...
        if video == "goat":
            #save img 
            cv2.imwrite('/root/autodl-tmp/code/Training-Code-of-STM/WarpImage/goat_img_{}.png'.format(t), cv2.cvtColor(warped_curr_frame_img, cv2.COLOR_RGB2BGR))
        if video == "blackswan":
            #save img 
            cv2.imwrite('/root/autodl-tmp/code/Training-Code-of-STM/WarpImage/blackswan_img_{}.png'.format(t), cv2.cvtColor(warped_curr_frame_img, cv2.COLOR_RGB2BGR))
        ...
        if video == "goat":
            #save img 
            sns.heatmap(normalized_photometric_loss_map, cmap='inferno')
            # Save the plot
            # Remove x and y axes
            plt.axis('off')
            plt.savefig('/root/autodl-tmp/code/Training-Code-of-STM/heatmao/goat_img_{}.png'.format(t), dpi=300)
            plt.close()
            # plt.imsave('/root/autodl-tmp/code/Training-Code-of-STM/heatmao/car_shadow_img_{}.png'.format(t), normalized_photometric_loss_map)
        if video == "blackswan":
            #save img 
            sns.heatmap(normalized_photometric_loss_map, cmap='inferno')
            # Remove x and y axes
            plt.axis('off')
            # Save the plot
            plt.savefig('/root/autodl-tmp/code/Training-Code-of-STM/heatmao/blackswan_img_{}.png'.format(t), dpi=300)
            # plt.imsave('/root/autodl-tmp/code/Training-Code-of-STM/heatmao/blackswan_img_{}.png'.format(t), normalized_photometric_loss_map)
            plt.close()
   	...
   	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        max_confidence_scores, _ = torch.max(torch.stack([E.to(device), estimate_curr_E.to(device)], dim=0), dim=0)
        pred[t] = torch.argmax(max_confidence_scores.to(device)[0], dim=0).cpu().numpy().astype(np.uint8)
